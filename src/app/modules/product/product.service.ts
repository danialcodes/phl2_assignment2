...
const updateSingleProductIntoDB = async (id:string, productData: IProduct) => {
    const result = await Product.findOneAndUpdate({_id:id}, productData,{new:true});
    return result;
};

const productUpdateWhileOrder = async (productId:string, quantity:number,totalPrice:number) => {
    ...
    if(!product){
        ...
        const productUpdateInfo:{quantity:number,inStock?:boolean} = {quantity: product.quantity - quantity};
        ...
        const result = await Product.findOneAndUpdate({_id:productId},productUpdateInfo,{new:true});
